...
# Qwen3-8B RL training with GSPO — FULLY ASYNC (off-policy)
# Algorithm: GSPO (GRPO with sequence-level KL)
...
SAVE_DIR="${SAVE_DIR:-/jpfs-5p/chenyanxu.9/model/Qwen3-8B-offpolicy-adam-gspo-${TIMESTAMP}}"
...
# ---- GSPO algorithm ----
# GSPO = GRPO + sequence-level KL (instead of per-token KL)
# Uses the same PPO-clip loss as DAPO, but KL is computed at sequence level
# via compute_gspo_kl, which requires all_gather of full log probs.
GSPO_ARGS=(
