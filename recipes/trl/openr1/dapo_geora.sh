...
OUTPUT_DIR=outputs/grpo_geora_qwen2_5_1_5b_$(date +%Y%m%d_%H%M%S)
...
CUDA_VISIBLE_DEVICES=0 ACCELERATE_LOG_LEVEL=info \
    ...
    --config.peft.type "geora" \
    --config.peft.task_type "CAUSAL_LM" \
    --config.peft.r 16 \
    --config.peft.lora_alpha 16 \
    --config.peft.lora_dropout 0.05 \
    --config.peft.sparsity_ratio 0.2 \
    --config.peft.hyper_param_type "LLM-Adapters" \
