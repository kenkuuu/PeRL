#!/bin/bash
# GRPO + LoRA training on Countdown with Qwen2.5-3B-Instruct (4x NVIDIA RTX A6000 48GB)

unset WANDB_DISABLED
OUTPUT_DIR=outputs/grpo_lora_qwen25_3b_countdown_$(date +%Y%m%d_%H%M%S)
...
CUDA_VISIBLE_DEVICES=0,1,2,3 ACCELERATE_LOG_LEVEL=info FLASHINFER_DISABLE_VERSION_CHECK=1 PYTORCH_ALLOC_CONF=expandable_segments:True \
    ...
    --config.logging.wandb_project "grpo-lora-countdown" \
    --config.dataset.dataset_name_or_path "countdown" \
