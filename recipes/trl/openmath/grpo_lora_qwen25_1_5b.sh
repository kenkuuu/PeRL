#!/bin/bash
# GRPO + LoRA training on OpenMath (DAPO-Math-17k) with Qwen2.5-1.5B-Instruct (4x NVIDIA RTX A6000 48GB)

unset WANDB_DISABLED
OUTPUT_DIR=outputs/grpo_lora_qwen25_1_5b_openmath_$(date +%Y%m%d_%H%M%S)
...
CUDA_VISIBLE_DEVICES=0,1,2,3 ACCELERATE_LOG_LEVEL=info FLASHINFER_DISABLE_VERSION_CHECK=1 PYTORCH_ALLOC_CONF=expandable_segments:True \
    ...
    --config.logging.wandb_project "grpo-lora-openmath" \
